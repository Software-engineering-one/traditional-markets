...
class FoodManagerApp:
    ...
    def add_item(self):
        ...
        messagebox.showinfo("알림", f"{name}가 음식란에 추가되었습니다.")
    ...
    def remove_item(self):
        ...
        # Optionally, you can display a message box or update a status label here
        messagebox.showinfo("알림", f"{name}가 음식란에서 삭제되었습니다.")
...
def click_button1():
    ...
    sublabel = tk.Label(tkin, text="1. 별점 및 평가 입력 2. 별점 및 평가 조회 3. 게시판조회 4. 평가 삭제")
    sublabel.pack()
    ...
    button5 = tk.Button(tkin, text="3. 게시판조회", command=click_button5)
    button5.pack()

    button6 = tk.Button(tkin, text="4. 평가 삭제", command=click_button6)
    button6.pack()
...
def click_button2():
    ...
    board_button = tk.Button(tkin, text="게시판확인", command=notice2)
    board_button.pack()
...
def notice():
    ...
    answer_button = tk.Button(tkin, text = "평가에 대한 답장", command=join)
    answer_button.pack()
...
def join():
    ...
    answer_label = tk.Label(tkin, text="답장:")
    answer_label.pack()
    answer_entry = tk.Entry(tkin)
    answer_entry.pack()
    answer_button = tk.Button(tkin, text="답장 완료", command=lambda: response(answer_entry.get()))
    answer_button.pack()

def response(reanswer):
    global answer
    answer = {reanswer:reanswer} # clova x참고
    click_button2()

def board():
    reanswer = list(answer.keys())[0]
    total_Label = tk.Label(tkin, text= f"{reanswer}")
    total_Label.pack() #clova x 참고
...
def notice2():
    for widget in tkin.winfo_children():
        widget.destroy()
    account()
    retry_label1 = tk.Label(tkin, account(), text = "에 대한 답장")
    retry_label1.pack()
    board()
    back_button = tk.Button(tkin, text="뒤로가기", command=click_button2)
    back_button.pack()
...
def click_button3():
    ...
    consumer_Label.pack()
    consumer_label1 = tk.Label(tkin, text="닉네임:")
    consumer_label1.pack()
    consumer_entry1 = tk.Entry(tkin)
    consumer_entry1.pack()
    ...
    consumer_entry3 = tk.Entry(tkin)
    consumer_entry3.pack()
    starandevalcheck = tk.Button(tkin, text="별점 및 평가 입력 완료", command=lambda: starandeval(consumer_entry1.get(), consumer_entry2.get(), consumer_entry3.get()))
    starandevalcheck.pack()

    back_button = tk.Button(tkin, text="뒤로가기", command=click_button1)
    back_button.pack()

def starandeval(nickname, star, eval):
    global rate
    rate[nickname] = {'star': star, 'eval': eval}
    click_button1()

def account():
    for nickname, review in rate.items():
        total_Label = tk.Label(tkin, text=f"{nickname}: {review['star']} - {review['eval']}")
        total_Label.pack()
...
def click_button6():
    ...
    delete_Label = tk.Label(tkin, text="삭제할 닉네임을 입력하세요")
    delete_Label.pack()
    delete_entry = tk.Entry(tkin)
    delete_entry.pack()
    delete_button = tk.Button(tkin, text="삭제", command=lambda: delete_evaluation(delete_entry.get()))
    delete_button.pack()
...
def delete_evaluation(nickname):
    if nickname in rate:
        del rate[nickname]
        messagebox.showinfo("알림", "평가가 삭제되었습니다.")
    else:
        messagebox.showerror("오류", "해당 닉네임이 존재하지 않습니다.")
    click_button1()
